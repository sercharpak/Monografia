...
results_path = '/lustre/home/ciencias/fisica/pregrado/sd.hernandez204/Outputs/graphics/10_03_2016/'
...
plt.title('Speed Histogram', fontsize=30)
...
for i in range(1,5):
    ...
    plt.tick_params(axis='both', which='major', labelsize=30)
    plt.grid()
    plt.title('Speed distribution for \n  z in [ '+str(l_z_old) + ' ; '+str(l_z_new)+' ]',fontsize=60)
    plt.xlabel('X kpc/h', fontsize=50)
    plt.ylabel('Y kpc/h', fontsize=50)
    cbar=plt.colorbar()
    cbar.set_label('Speed',size=50)
...
for i in range(1,5):
    ...
    if(n_galaxies>0):
        ...
        plt.tick_params(axis='both', which='major', labelsize=30)
        plt.grid()
        plt.title('Speed lesser than '+ str(thresh_v_low)+' \n for z between '+str(l_z_old) + ' and '+str(l_z_new),fontsize=40)
        plt.xlabel('X kpc/h', fontsize=40)
        plt.ylabel('Y kpc/h', fontsize=40)
        cbar=plt.colorbar()
        cbar.set_label('Speed',size=40)
