...
void kdReadAscii(KD kd,FILE *fp,int bDark,int bGas,int bStar)
{
	int i,j,nCnt;
	struct dump h;
	struct gas_particle gp;
	...
	fscanf(fp, "%d", &kd->nParticles);
	printf("%d\n", kd->nParticles);
	fscanf(fp, "%d", &kd->nDark);
	fscanf(fp, "%d", &kd->nGas);
	fscanf(fp, "%d", &kd->nStar);
	fscanf(fp, "%g", &kd->fTime);
	fscanf(fp, "%d", &kd->nActive);
	if (bDark) kd->nActive += kd->nDark;
	if (bGas) kd->nActive += kd->nGas;
	if (bStar) kd->nActive += kd->nStar;
	kd->bDark = bDark;
	...
	nCnt = 0;
	for (i=0;i<kd->nGas;++i) {
		fscanf(fp, "%g %g %g", &gp.pos[0], &gp.pos[1], &gp.pos[2]);
		if (bGas) {
			kd->p[nCnt].iOrder = nCnt;
			for (j=0;j<3;++j) kd->p[nCnt].r[j] = gp.pos[j];
			++nCnt;
	...
	for (i=0;i<kd->nDark;++i) {
		fscanf(fp, "%g %g %g", &dp.pos[0], &dp.pos[1], &dp.pos[2]);
		if (bDark) {
			kd->p[nCnt].iOrder = nCnt;
			for (j=0;j<3;++j) kd->p[nCnt].r[j] = dp.pos[j];
			++nCnt;
	...
	for (i=0;i<kd->nStar;++i) {
	        fscanf(fp, "%g %g %g", &sp.pos[0], &sp.pos[1], &sp.pos[2]);
		if (bStar) {
			kd->p[nCnt].iOrder = nCnt;
			for (j=0;j<3;++j) kd->p[nCnt].r[j] = sp.pos[j];
			++nCnt;
